...
public class MenuController {
    ...
    @GetMapping("/{username}")
    public IBotResponse getUserRouters(@NotBlank(message = "{required}") @PathVariable String username) {
        ...
        return new IBotResponse().data(result);
    }

    @GetMapping
    public IBotResponse menuList(Menu menu) {
        Map<String, Object> menus = this.menuService.findMenus(menu);
        return new IBotResponse().data(menus);
    }
