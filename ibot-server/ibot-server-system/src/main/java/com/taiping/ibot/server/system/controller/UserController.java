...
public class UserController {
    ...
    public IBotResponse index() {
        ...
        param.setUsername(IBotUtil.getCurrentUsername());
    ...
    public IBotResponse userList(QueryRequest queryRequest, SystemUser user) {
        Map<String, Object> dataTable = IBotUtil.getDataTable(userService.findUserDetailList(user, queryRequest));
    ...
    @GetMapping("/{userId}")
    @PreAuthorize("hasAuthority('user:update')")
    public IBotResponse findUserDataPermissions(@NotBlank(message = "{required}") @PathVariable String userId) {
        String dataPermissions = this.userDataPermissionService.findByUserId(userId);
        return new IBotResponse().data(dataPermissions);
    }
    ...
    public void updateProfile(@Valid SystemUser user) throws IBotException {
